...
public class TimeSlotScheduler {
	...
	private ScheduledTimetableEntryList computeCourseLearningSlots(
		...
		if (freeTimeSlots.size() > 0
			...
			// double accumulatedFreeTimeHours =
			// getAccumulatedFreeTimeHours(freeTimeSlots);
			// int coursePeriodNumberOfDays = DateUtility.getDaysOfDuration(new
			// DateTime(), course.getDeadline());
			// double averageFreeTimeHoursPerDay = accumulatedFreeTimeHours /
			// coursePeriodNumberOfDays;
			...
			// double averageNeededHoursPerDay = hoursToSchedule /
			// coursePeriodNumberOfDays;
