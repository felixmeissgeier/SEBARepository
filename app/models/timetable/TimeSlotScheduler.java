...
public class TimeSlotScheduler {
	...
	currentCourseColor = new Color((int) (Math.random() * 250), (int) (Math.random() * 250),
		(int) (Math.random() * 250));
	...
	if (course.getCourseMaterial().isConsiderExpectedHours()) {
	    ...
	    hoursToSchedule = (course.getDifficulty() / DIFFICULTY_RELATION_PAGES_PER_X_HOURS)
		    * course.getCourseMaterial().getScriptPagesToDo();
	}
	...
	for (int i = 0; i < freeTimeSlots.size(); i++) {
		...
		// daytime
		DateTime bestStartTime = DateUtility.getBestDayTimeMatch(currentInterval.getStartDateTime(),
			currentInterval.getEndDateTime(), customerPreferences.getPreferredLearningDayTime());
		if (bestStartTime != null) {
		    ...
		    if (customerPreferences.getMaxLearningSlotDuration() < hoursToSchedule
			    && intervalHours >= customerPreferences.getMaxLearningSlotDuration()) {
			...
			scheduledWorkLoad.add(new TimetableEntry(course.getTitle(), "Learning Slot", currentInterval
				.getStartDateTime(), currentInterval.getStartDateTime().plusMinutes(
				learningSlotDurationMinutes), TimetableEntryType.SCHEDULED_LEARNINGSLOT,
				currentCourseColor));
		    } else {
	...
	if (hoursToSchedule > 0) {
		...
		while (scheduledWorkLoad.get(timetableEntryIterator).getStartDateTime().getDayOfYear() < currentDateTime
			.getDayOfYear() && (timetableEntryIterator < scheduledWorkLoad.size() - 1)) {
		...
		if (timetableEntryDay > currentDateTime.getDayOfYear()) {
			...
			if (interval.getStartDateTime().getDayOfYear() == currentDateTime.getDayOfYear()) {
			    ...
			    if (learningSlotDurationMinutes != 0) {
				hoursToSchedule -= learningSlotDurationMinutes / 60.0;
				scheduledWorkLoad.add(new TimetableEntry(course.getTitle(), "Learning Slot", interval
					.getStartDateTime(), interval.getStartDateTime().plusMinutes(
					learningSlotDurationMinutes), TimetableEntryType.SCHEDULED_LEARNINGSLOT,
					currentCourseColor));
				break;
			    }
	...
	if (hoursToSchedule > 0) {
		...
		if (hoursToSchedule > 0 && interval.getDurationInHours() >= minSlotDuration
			&& day == interval.getStartDateTime().getDayOfYear()) {
		    int duration = 0;
		    ...
		    currentTimetable.add(new TimetableEntry(course.getTitle(), "Learning Slot", interval
			    .getStartDateTime(), interval.getStartDateTime().plusMinutes(duration),
	...
	for (DateTimeInterval currentInterval : freeTimeSlots) {
	    accumulatedHours += DateUtility.getMinutesOfDuration(currentInterval.getStartDateTime(),
		    currentInterval.getEndDateTime()) / 60.0;
	}
	...
	if (restTimeIntervals != null) {
		...
		if (restDays != null) {
			...
			TimetableEntry blockEntry = new TimetableEntry("Day Of Rest", "", startDateTime, endDateTime,
				TimetableEntryType.BLOCKED, null);
		...
		for (TimeInterval timeInterval : restTimeIntervals) {
		    DateTime startDateTime = new DateTime(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth(),
			    timeInterval.getTime1().getHourOfDay(), timeInterval.getTime1().getMinuteOfHour());
		    DateTime endDateTime = new DateTime(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth(),
			    timeInterval.getTime2().getHourOfDay(), timeInterval.getTime2().getMinuteOfHour());
		    TimetableEntry blockEntry = new TimetableEntry("Free Time", "", startDateTime, endDateTime,
			    TimetableEntryType.BLOCKED, null);
		    extendedTimetableEntries.add(blockEntry);
		}
	    }
	...
	if (currentCalendarData != null) {
		...
		if (entry.getStartDateTime().isAfter(potentialTimeSlotBegin)
			&& entry.getStartDateTime().isBefore(deadline) && entry.getEndDateTime().isBefore(deadline)) {
			...
			for (int i = potentialTimeSlotBegin.getDayOfYear() + 1; i < entry.getStartDateTime()
				.getDayOfYear(); i++) {
			    DateTime timeSlotBeginBetween = (new DateTime()).withDayOfYear(i).withHourOfDay(0)
				    .withMinuteOfHour(0);
